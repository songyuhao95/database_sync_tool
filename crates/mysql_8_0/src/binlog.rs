...
pub fn validate_change_event(tx: ChangeTransaction) -> io::Result<ValidatedTransaction> {
    ...
    mysql_source_contract::validate_source(&validated, mysql_source_contract::MysqlVersion::V80)
...
pub fn binlog(config: BinlogConfig) -> io::Result<BinlogStream> {
    ...
    if !version.starts_with("8.0.") {
        return Err(io::Error::other(format!(
            "mysql_8_0 requires MySQL 8.0; source is {version}"
        )));
    }
    if log_bin != 1 || format != "ROW" || image != "FULL" {
        return Err(io::Error::other(
            "mysql_8_0 requires log_bin=ON, binlog_format=ROW, binlog_row_image=FULL",
        ));
    }
    ...
    for (name, value) in modes {
        ...
        {
            return Err(io::Error::other(format!(
                "mysql_8_0 currently requires full JSON images and uncompressed transactions; {name}={value}"
            )));
        }
    }
