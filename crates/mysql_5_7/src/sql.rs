...
/// Convert one complete, validated ChangeEvent transaction to a MySQL 5.7 plan.
pub fn sql(validated: &ValidatedTransaction) -> io::Result<SqlTransaction> {
    ...
    for change in &transaction.changes {
        ...
        let (sql, params) = SqlRenderer.write_change(change)?;
...
/// Render a transaction using the immutable ColumnConversionPlans selected by
/// route activation.  The legacy [`sql`] entry point remains available for
/// callers that have not adopted plan-backed routing, but production workers
/// should use this function so conversion and capability decisions cannot be
...
fn planned_sql(
    ...
    for change in &transaction.changes {
        ...
        let (sql, params) = SqlRenderer.write_change_with_plans(change, plans)?;
        let probe = match change.operation {
            Operation::Insert => None,
            Operation::Update | Operation::Delete => {
...
/// Execute the already rendered SQL as one target transaction.
...
fn bind_logical_value(value: &LogicalValue) -> io::Result<Value> {
    match value {
        ...
        | LogicalValue::Raw { .. } => Err(capability_failure(
            "MySQL 5.7 has no qualified target representation for this structured value",
        )),
