...
pub fn validate_change_event(tx: ChangeTransaction) -> io::Result<ValidatedTransaction> {
    ...
    mysql_source_contract::validate_source(&validated, mysql_source_contract::MysqlVersion::V84)
...
pub fn binlog(config: BinlogConfig) -> io::Result<BinlogStream> {
    ...
    if !version.starts_with("8.4.") {
        return Err(io::Error::other(format!(
            "mysql_8_4 requires MySQL 8.4; source is {version}"
        )));
    }
    if log_bin != 1 || format != "ROW" || image != "FULL" {
        return Err(io::Error::other(
            "mysql_8_4 requires log_bin=ON, binlog_format=ROW, binlog_row_image=FULL",
        ));
    }
    // These modes need reconstruction beyond complete row images.
    let modes: Vec<(String, String)> = conn.query(
        "SHOW GLOBAL VARIABLES WHERE Variable_name IN ('binlog_row_value_options', 'binlog_transaction_compression')"
    ).map_err(io::Error::other)?;
    for (name, value) in modes {
        if (name == "binlog_row_value_options" && !value.is_empty())
            || (name == "binlog_transaction_compression" && value == "ON")
        {
            return Err(io::Error::other(format!(
                "mysql_8_4 currently requires full JSON images and uncompressed transactions; {name}={value}"
            )));
        }
    }
    let (file, position, _, _, executed_gtids): (String, u64, String, String, String) = conn
        ...
        .map_err(io::Error::other)?
        .ok_or_else(|| io::Error::other("SHOW BINARY LOG STATUS returned no binlog"))?;
    ...
    let mut raw_log = config
        ...
        .map(|path| {
            OpenOptions::new()
                ...
                .open(&path)
                .map_err(|error| {
                    io::Error::other(format!("could not open {}: {error}", path.display()))
                })
                .map(BufWriter::new)
        })
        .transpose()?;
    ...
    conn.query_drop("SET SESSION time_zone = '+00:00'")
        .map_err(io::Error::other)?;
    conn.query_drop("SET @source_heartbeat_period = 1000000000")
        .map_err(io::Error::other)?;
