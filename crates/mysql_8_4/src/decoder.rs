    ...
    pub(crate) fn decode(&mut self, event: &Event) -> io::Result<Option<ChangeTransaction>> {
        ...
        match event.read_data() {
            ...
            Ok(Some(EventData::GtidEvent(gtid))) => self
                .start_transaction(transaction_id(&gtid), event_cursor)
                .map(|_| None),
            ...
            Ok(Some(other)) => Err(io::Error::other(format!(
                "unsupported 8.4 binlog event: {other:?}"
            ))),
...
pub(crate) fn decode_mysql_value(value: &Value, info: &ColumnInfo) -> io::Result<LogicalValue> {
    ...
    match value {
        Value::Int(value) => {
            ...
            // MINIMAL TABLE_MAP metadata may omit signedness; use the captured column type.
...
fn transaction_id(gtid: &mysql_driver::binlog::events::GtidEvent) -> String {
    let plain = format_gtid(gtid.sid(), gtid.gno());
    match gtid.tag() {
        Some(tag) => {
            let (sid, number) = plain.rsplit_once(':').expect("GTID has a separator");
            format!("{sid}:{}:{number}", tag.as_str())
        }
        None => plain,
    }
}
